...
void node_implementation_init(nmos::node_model& model, slog::base_gate& gate)
{
    ...
    for (int index = 0; index < how_many; ++index)
    {
        for (const auto& port : impl::ports::rtp)
        {
            ...
            if (impl::ports::video == port)
            {
                ...
                // add video/H265 format parameters to flow.data
                // i.e. nmos::fields::profile_id, etc.
            }
            else if (impl::ports::audio == port)
            {
            ...
            if (impl::ports::video == port)
            {
                // add video/H265 transport parameters to sender.data?
            }
    ...
    for (int index = 0; index < how_many; ++index)
    {
        for (const auto& port : impl::ports::rtp)
        {
            ...
            if (impl::ports::video == port)
            {
                receiver = nmos::make_receiver(receiver_id, device_id, nmos::transports::rtp, interface_names, nmos::formats::video, { nmos::media_types::video_H265 }, model.settings);
...
nmos::transport_file_parser make_node_implementation_transport_file_parser()
{
    return [](const nmos::resource& receiver, const nmos::resource& connection_receiver, const utility::string_t& transport_file_type, const utility::string_t& transport_file_data, slog::base_gate& gate)
    {
        ...
        {
            if (nmos::media_types::video_H265 == nmos::get_media_type(sdp_params))
            {
    ...
    return [&node_resources, node_id, rtp_source_ids, rtp_flow_ids, rtp_sender_ids](const nmos::resource& sender, const nmos::resource& connection_sender, value& endpoint_transportfile)
    {
        ...
        if (rtp_sender_ids.end() != found)
        {
            ...
            auto sdp_params = [&]
            {
                ...
                {
                    const nmos::video_H265_parameters params{
                        (uint32_t)nmos::fields::profile_id(flow->data),
                        (uint32_t)nmos::fields::profile_space(flow->data),
                        (uint32_t)nmos::fields::level_id(flow->data),
                        (uint32_t)nmos::fields::tier_flag(flow->data),
                        // should add remaining video/H265 format-specific parameters
                        {}, {}, {}, {}
                    };
